...
class BetController {
    constructor() {
        this.bet = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (gameKind === 'sports' || gameKind === 'sportsCross' || gameKind === 'sportsLive' || gameKind === 'sportsSpecial' || gameKind === 'sportsLiveKor') {
                ...
                if (gameKind === 'powerball')
                    ...
                    category = 'boglePowerballBet';
                else if (gameKind === 'bogleladder')
                    category = 'bogleladderBet';
        ...
        this.betSports = (v, res, data) => __awaiter(this, void 0, void 0, function* () {
            ...
            let gameKindForBonus = '';
            if (v.gameKind === 'sports')
                gameKindForBonus = 'prematchEurope';
            else if (v.gameKind === 'sportsCross')
                gameKindForBonus = 'prematchKor';
            else if (v.gameKind === 'sportsLive')
                gameKindForBonus = 'live';
            else if (v.gameKind === 'sportsSpecial')
                gameKindForBonus = 'special';
            else if (v.gameKind === 'sportsLiveKor')
                gameKindForBonus = 'realtime';
            //■■■■■■■■■■ DB-보너스 배당 정보 가져오기 ■■■■■■■■■■
            let rBonus = yield betService.betBonus(gameKindForBonus, v.betCart.length);
            if (rBonus.error) {
                res.status(500).end();
                return;
            }
            ...
            if (config_1.default.db.id === 'pent') {
                if (v.betCount === 1) {
                    ...
                    let max = modules_1._.maxBy(v.betCart, function (o) { return o.finalRate; });
                    let indexMax = v.betCart.findIndex((x) => { return x._id === max._id; });
                    ...
                    if (overRate < 2) {
                        ...
                        // console.log('=========================')
                        while (true) {
                            let sumRateForMax = bonusRate ? bonusRate : 1;
        ...
        this.betMiniGame = (v, res, data) => __awaiter(this, void 0, void 0, function* () {
            //해당게임 배팅가능 여부 검사
            let gameKind = v.betCart[0].gameKind;
            v.gameKind = gameKind;
            let categorySwitch = '';
            ...
            const resultBetSwitch = yield betService.betSwitch(v);
            ...
            if (!v.resultBetSwitch.betStatus) {
                data.errorTitle = '배팅 실패 - 400';
                data = tools_1.default.denyValidate(data, 'betCart', '현재 배팅이 불가능한 게임입니다.');
                res.status(400).json(data);
                return;
            }
            //■■■■■■■■■■ DB-미니게임 게임정보 가져오기 ■■■■■■■■■■
            const resultMinigameInfo = yield betService.minigameInfo(v);
            ...
            if (!v.resultMinigameInfo) {
                data.errorTitle = '배팅 실패 - 400';
                data = tools_1.default.denyValidate(data, 'betCart', '진행 중인 경기가 없습니다.');
                res.status(400).json(data);
                return;
            }
            //최대 당첨금 검사
            ...
            const resultPreviousBetAmount = yield betService.previousBetAmount(v);
            ...
            //■■■■■■■■■■ DB-미니게임 배팅 ■■■■■■■■■■
            const resultBetMinigame = yield betService.betMinigame(v);
            ...
            if (!v.resultBetSubtractMinigame.isTest) {
                const resultSetBetMinigame = yield betService.setBetMinigame(v);
            ...
            //■■■■■■■■■■ DB-미니게임 경기에 배팅금액 업데이트 ■■■■■■■■■■
            //■■■■■■■■■■ DB-미니게임 배팅 로그 남기기 ■■■■■■■■■■
            let resultSetBetMoneyLog = yield betService.setBetMoneyLog(v);
        ...
        this.betMiniGamePent = (v, res, data) => __awaiter(this, void 0, void 0, function* () {
            //해당게임 배팅가능 여부 검사
            let gameKind = v.betCart[0].gameKind;
            v.gameKind = gameKind;
            let categorySwitch = '';
            ...
            //■■■■■■■■■■ DB-미니게임 배팅머니 차감, 회원정보 업데이트, 회원정보 가져오기 ■■■■■■■■■■
            const resultBetSubtractMinigame = yield betService.betSubtractMinigamePent(v);
            if (resultBetSubtractMinigame.error) {
                res.status(500).json(data);
                return;
            }
            ...
            let getKeys = ['salary'];
            // ■■■■■■■■■■ DB-회원정보 가져오기 ■■■■■■■■■■
            let rUserInfo = yield userService.userInfo(topOID, getKeys);
            if (rUserInfo.error) {
            ...
            const betTotalRollingRate = v.topSalaryInfo[gameKind][`${v.betCart[0].betType.replace(/PWB/gi, '').replace(/PLD/gi, '').replace(/KLD/gi, '').replace(/BLD/gi, '')}Rate`];
            const betTotalRolling = parseInt((v.betAmount * (betTotalRollingRate / 100)).toString());
            //■■■■■■■■■■ DB-전체 상부 상태, 요율 가져오기 ■■■■■■■■■■
            let resultBetTopInfo = yield betService.betTopInfo(v);
            ...
            //상부 설정
            //상부  올리기 T/F 전체설정
            let betTopStatusTotal = v.resultBetTopInfo[`${v.betCart[0].betType.replace(/PWB/gi, '').replace(/PLD/gi, '').replace(/KLD/gi, '').replace(/BLD/gi, '')}Status`];
            //상부  올리기 T/F 개인설정
            let betTopStatusPerson = v.resultBetSubtractMinigame.topConfig[v.gameKind][`${v.betCart[0].betType.replace(/PWB/gi, '').replace(/PLD/gi, '').replace(/KLD/gi, '').replace(/BLD/gi, '')}Status`];
            let betTopStatus = false;
            ...
            let betKillAmount = 0;
            if (betTopStatus) {
                let betTopRateTotal = v.resultBetTopInfo[`${v.betCart[0].betType.replace(/PWB/gi, '').replace(/PLD/gi, '').replace(/KLD/gi, '').replace(/BLD/gi, '')}Rate`];
                let betTopRatePerson = v.resultBetSubtractMinigame.topConfig[v.gameKind][`${v.betCart[0].betType.replace(/PWB/gi, '').replace(/PLD/gi, '').replace(/KLD/gi, '').replace(/BLD/gi, '')}Rate`];
                //상부 올릴 %
            ...
            //■■■■■■■■■■ DB-미니게임 배팅 ■■■■■■■■■■
            const resultBetMinigame = yield betService.betMinigamePent(v);
            if (resultBetMinigame.error) {
                data.errorTitle = '배팅 실패 - 500';
                res.status(500).json(data);
                return;
            ...
            //■■■■■■■■■■ DB-미니게임 경기에 배팅금액 업데이트 ■■■■■■■■■■
            //■■■■■■■■■■ DB-미니게임 배팅 로그 남기기 ■■■■■■■■■■
            let resultSetBetMoneyLog = yield betService.setBetMoneyLogPent(v);
            // if(resultSetBetMoneyLog.error) {
            //     data.errorTitle = '배팅 실패 - 500'
            //     res.status(500).json(data)
            //     return
        ...
        this.getSportsBetList = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                page: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 가져오기 ■■■■■■■■■■
                const r = yield betService.getSportsBetList(v.page, v.decoded._id);
                if (r.error) {
        ...
        this.getMinigamesBetList = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                page: {
                    value: req.query.page,
                    rule: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 가져오기 ■■■■■■■■■■
                const r = yield betService.getMinigamesBetList(v.page, v.decoded._id);
                if (r.error) {
                    data.errorTitle = '배팅 내역 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.getCasinoBetList = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                page: {
                    value: req.query.page,
                    rule: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 가져오기 ■■■■■■■■■■
                const r = yield betService.getCasinoBetList(v.page, v.decoded._id);
                if (r.error) {
                    data.errorTitle = '배팅 내역 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.cancelSportsBet = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                _id: {
                    value: req.body._id,
                    rule: {
            ...
            try {
                ...
                // ■■■■■■■■■■ DB-배팅 카트 취소 ■■■■■■■■■■
                const resultSportsCartCancel = yield betService.sportsCartCancel(v);
                ...
                // ■■■■■■■■■■ DB-스포츠 경기에 업데이트 ■■■■■■■■■■
                yield betService.subtractBetSports(v);
                // ■■■■■■■■■■ DB-스포츠 경기에 업데이트 ■■■■■■■■■■
                // ■■■■■■■■■■ DB-회원머니 적립 및 배팅카운트 수정 ■■■■■■■■■■
                const resultSportsCartCancelUpdateUser = yield betService.sportsCartCancelUpdateUser(v);
                ...
                // ■■■■■■■■■■ DB-배팅취소 로그 ■■■■■■■■■■
                yield betService.sportsCartCancelMoneyLog(v);
                // ■■■■■■■■■■ DB-배팅취소 로그 ■■■■■■■■■■
                res.end();
            }
            catch (e) {
        ...
        this.deleteSportsBet = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                _id: {
                    value: req.query._id,
                    rule: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.sportsBetDelete(v);
                if (r.error) {
        ...
        this.deleteMinigamesBet = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                _id: {
                    value: req.query._id,
                    rule: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.minigamesBetDelete(v);
                if (r.error) {
                    data.errorTitle = '배팅 내역 삭제 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.deleteCasinoBet = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const validateData = {
                _id: {
                    value: req.query._id,
                    rule: {
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.casinoBetDelete(v);
                if (r.error) {
                    data.errorTitle = '배팅 내역 삭제 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.deleteSportsBetAll = (req, res) => __awaiter(this, void 0, void 0, function* () {
            // validate start
            ...
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.deleteSportsBetAll(v);
                if (r.error) {
                    data.errorTitle = '배팅 내역 삭제 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.deleteMinigamesBetAll = (req, res) => __awaiter(this, void 0, void 0, function* () {
            // validate start
            let v = tools_1.default.generateReqValue({}, req);
            let data = v;
            // validate end
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.deleteMinigamesBetAll(v);
                if (r.error) {
                    data.errorTitle = '배팅 내역 삭제 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.deleteCasinoBetAll = (req, res) => __awaiter(this, void 0, void 0, function* () {
            // validate start
            let v = tools_1.default.generateReqValue({}, req);
            let data = v;
            // validate end
            try {
                // ■■■■■■■■■■ DB-배팅 내역 삭제 ■■■■■■■■■■
                const r = yield betService.deleteCasinoBetAll(v);
                if (r.error) {
                    data.errorTitle = '배팅 내역 삭제 실패 - 500';
                    res.status(500).json(data);
                    return;
        ...
        this.getMinigameBetListRecent = (req, res) => __awaiter(this, void 0, void 0, function* () {
            // validate start
            let v = tools_1.default.generateReqValue({}, req);
            let data = v;
            // validate end
            try {
                // ■■■■■■■■■■ DB-미니게임 배팅 내역 ■■■■■■■■■■
                const r = yield betService.getMinigameBetListRecent(v.decoded._id);
                if (r.error) {
