...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            max() {
                const arraySportsRouter = ['Sports', 'SportsCross', 'SportsLive', 'SportsSpecial', 'SportsLiveKor']

                if(arraySportsRouter.indexOf(this.router) > -1) {
                    //console.log(this.betInfo)

                    let rBenefit = 0
                    let tmpBetMoney = this.user.money

                    rBenefit = parseInt((this.betRate * tmpBetMoney).toFixed(0))

                    if(rBenefit > this.betInfo.benefit) {
                        // 당첨 상한금으로 금액 맞추기.
                        let adjustBetMoney = Math.floor(this.betInfo.benefit / this.betRate)

                        if(adjustBetMoney > this.user.money) {
                            document.querySelector('#betAmount').value = this.$numeral(this.user.money).format('0,0')
                            document.querySelector('#betAmountMobile').value = this.$numeral(this.user.money).format('0,0')
                            // document.querySelector('#betAmountMinigame').value = this.$numeral(this.user.money).format('0,0')
                            ...
                            document.querySelector('#betAmount').value = this.$numeral(adjustBetMoney).format('0,0')
                            document.querySelector('#betAmountMobile').value = this.$numeral(adjustBetMoney).format('0,0')
                            // document.querySelector('#betAmountMinigame').value = this.$numeral(adjustBetMoney).format('0,0')

                            this.betAmount = this.$numeral(adjustBetMoney).format('0,0')
                        }
                    }
                    else {
                        if(this.user.money >= this.betInfo.max) {
                            document.querySelector('#betAmount').value = this.$numeral(this.betInfo.max).format('0,0')
                            document.querySelector('#betAmountMobile').value = this.$numeral(this.betInfo.max).format('0,0')
                            // document.querySelector('#betAmountMinigame').value = this.$numeral(this.betInfo.max).format('0,0')

                            this.betAmount = this.$numeral(this.betInfo.max).format('0,0')
                        }
                        else {
                            document.querySelector('#betAmount').value = this.$numeral(this.user.money).format('0,0')
                            document.querySelector('#betAmountMobile').value = this.$numeral(this.user.money).format('0,0')
