<template>
    ...
    >
        <div class="mb-1 sports-detail-title">
            <font-awesome-icon :icon="['fa', 'chevron-circle-down']" class="text-warning"/> 9이닝 언더오버
        </div>
        <div class="row" :class="index > 0 && v.games.underOver9thInning[index].showStatus && v.games.underOver9thInning[index].status !== 'HIDE' ? 'mt-1' : ''" v-for="(vv, index) in v.games.underOver9thInning" :key="index">
            <div class="col-5 px-1" v-if="v.games.underOver9thInning[index].showStatus && v.games.underOver9thInning[index].status !== 'HIDE'">
                <div
                    ...
                    @click="v.games.underOver9thInning[index].status === 'ACTIVE' ? $emit('setBet', {
                        _id: v._id,
                        sport: v.sport,
            ...
            <div class="col-2 px-1" v-if="v.games.underOver9thInning[index].showStatus && v.games.underOver9thInning[index].status !== 'HIDE'">
                <div
                    class="g-x"
                    :class="{
                        'n': v.games.underOver9thInning[index].status !== 'ACTIVE',
