<template>
    ...
    >
        <div class="mb-1 sports-detail-title">
            <font-awesome-icon :icon="['fa', 'chevron-circle-down']" class="text-warning"/> 승무패 핸디캡
        </div>
        <div class="row" :class="index > 0 && v.games.threeWayHandicap[index].showStatus && v.games.threeWayHandicap[index].status !== 'HIDE' ? 'mt-1' : ''" v-for="(vv, index) in v.games.threeWayHandicap" :key="index">
            <div class="col-5 px-1" v-if="v.games.threeWayHandicap[index].showStatus && v.games.threeWayHandicap[index].status !== 'HIDE'">
                <div
                    ...
                    @click="v.games.threeWayHandicap[index].status === 'ACTIVE' ? $emit('setBet', {
                        _id: v._id,
                        sport: v.sport,
            ...
            <div class="col-2 px-1" v-if="v.games.threeWayHandicap[index].showStatus && v.games.threeWayHandicap[index].status !== 'HIDE'">
                <div
                    class="g-x can-bet"
                    :class="{
                        'n': v.games.threeWayHandicap[index].status !== 'ACTIVE',
                ...
                >
                    ...
                    <span class="text-primary ml-sm-1 d-block d-sm-inline">
                        ({{ vv.homeStandard }})
                    </span>
                </div>
            </div>
